...
#
# File name:        Vec_Compare_Less_Than.asm
# Version:          1.0
# Date:             Apr. 28, 2015  
...
main:     
	   ...
	   addi $t4, $zero, 0x00			#if not less than set t4 to 0x00
...
compare1:  
	   ...
	   bne $t7, $zero, else1			#if less than branch to else1
	   addi $t4, $zero, 0x00			#if not less than set t4 to 0x00
...
compare2:  
	   ...
	   sltu $t7, $t2, $t3				#compare t2 < t3 and store result in t7 
	   bne $t7, $zero, else2			#if less than branch to else2
	   addi $t4, $zero, 0x00			#if not less than set t4 to 0x00 
...
compare3: 
	   ...
	   sltu $t7, $t2, $t3				#compare t2 < t3 and store result in t7
	   bne $t7, $zero, else3			#if less than branch to else3
	   addi $t4, $zero, 0x00			#if not less than set t4 to 0x00
...
compare4: 
	   ...
	   sltu $t7, $t2, $t3				#compare t2 < t3 and store result in t7
	   bne $t7, $zero, else4			#if less than branch to else4
	   addi $t4, $zero, 0x00			#if not less than set t4 to 0x00
...
compare5: 
	   ...
	   sltu $t7, $t2, $t3				#compare t2 < t3 and store result in t7
	   bne $t7, $zero, else5			#if less than branch to else5
	   addi $t4, $zero, 0x00			#if not less than set t4 to 0x00
...
compare6: 
	   ...
	   sltu $t7, $t2, $t3				#compare t2 < t3 and store result in t7
	   bne $t7, $zero, else6			#if less than branch to else6
	   addi $t4, $zero, 0x00			#if not less than set t4 to 0x00
...
compare7:  sll $t4, $t4, 08				#shift result of t4 to correct position 
	   ...
	   sltu $t7, $t2, $t3				#compare t2<t3 and store result in t7
	   bne $t7, $zero, else7			#if less than branch to else7
	   addi $t4, $zero, 0x00			#if not less than set t4 to 0x00
