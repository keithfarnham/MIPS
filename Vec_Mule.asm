...
#
# Description: Each element of vector d is the full-length (16-bit) product of the corresponding high (i.e. even) 
...
main:     
	   ...
	   li $a0, 0xAEE95AE0				#initialize a0 with upper bits of vec a
	   li $a1, 0xF080CC66				#initialize a1 with lower bits of vec a
	   li $a2, 0x33146170				#initialize a2 with upper bits of vec b
	   li $a3, 0x609888AB				#initialize a3 with lower bits of vec b
	   ...
	   lw $t2, 0($t0)  				#load first even 8 bits of vec a into t2
	   lw $t3, 0($t1)				#load first even 8 bits of vec b into t3
	   mul $t5, $t2, $t3				#multiply first even 8 bits of a and b and store into t5
	   sll $t5, $t5, 16				#shift result to first 16 bits
	   
	   lw $t2, 8($t0) 				#load second even 8 bits of vec a into t2
	   lw $t3, 8($t1)				#load second even 8 bits of vec b into t2
	   mul $t7, $t2, $t3				#multiply second even 8 bits of a and b and store into t5
	   add $t5, $t5, $t7				#move product from temp reg to t5
	   
	   lw $t2, 16($t0)  				#load third even 8 bits of vec a into t2
	   lw $t3, 16($t1)				#load third even 8 bits of vec a into t2
	   mul $t6, $t2, $t3				#multiply third even 8 bits of a and b and store into t6
	   sll $t6, $t6, 16				
	   
	   lw $t2, 24($t0) 				#load last even 8 bits of vec a into t2
	   lw $t3, 24($t1)				#load last even 8 bits of vec a into t2
